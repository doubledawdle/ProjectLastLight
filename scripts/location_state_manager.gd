...
func update_player_resource(resource_type: String, rangeX: float, rangeY:float, time: int) -> void: 
	...
	timer.one_shot = true
	timer.connect("timeout", Callable(self, "_on_timer_timeout").bind(resource_info))
...
func _on_timer_timeout(resource_info: Dictionary) -> void:
	...
	ResourceInventory.add_resource(ResourceInventory.ResourceType.VILLAGERS, 1)
